...
import { USER_TYPES } from '../services/firebaseStructure'; // Assumindo este path
import CepService from '../services/cepService'; // Assumindo este path
...
const RegisterOlheiroScreen = ({ navigation }) => {
        ...
        if (cepLimpo.length === 8) {
            ...
            try {
                ...
                if (result.success && !result.data.erro) {
                    ...
                    // Limpa apenas os campos de endereço se o CEP não for encontrado
            ...
            // Limpa os campos de endereço se o usuário apagar o CEP
            updateFormData('estado', '');
            updateFormData('cidade', '');
            updateFormData('rua', '');
        }
    ...
    const validateStep = (step) => {
        ...
        switch (step) {
            ...
            case 3:
                ...
                const isEmailValid = email.trim().includes('@'); // Validação simples
    ...
    const handleRegister = async () => {
        ...
        try {
            ...
            const result = await register(formData.email.trim(), formData.senha, userData, USER_TYPES.OLHEIRO);

            if (result.success) {
                Alert.alert('Sucesso 🎉', 'Seu cadastro foi concluído!');
                // A navegação para a tela principal (Home) será feita automaticamente pelo AuthContext após o login
            } else {
                ...
                <View style={styles.buttonContainer}>
                    ...
                    <TouchableOpacity
                        style={[
                            ...
                            currentStep === 1 && { flex: 1, marginLeft: 0 } // Ajuste para centralizar no passo 1
                        ]}
                        onPress={handleNext}
...
const styles = StyleSheet.create({
    ...
    headerContent: {
        ...
        left: 0,
        right: 0,
        // Garante que o conteúdo não cubra o botão de voltar, ajustando a margem
        marginLeft: 40, 
        marginRight: 40,
    },
    successIcon: {
    ...
    progressSteps: {
        ...
        alignItems: 'center',
        paddingHorizontal: 15, // Ajuste para centralizar os círculos
    },
    stepContainer: {
        flexDirection: 'row',
        alignItems: 'center',
    ...
    stepLine: {
        ...
        marginHorizontal: -5, // Para compensar o padding do progressSteps e alinhar com os círculos
    },
    stepLineActive: {
        backgroundColor: colors.primary,
    },
