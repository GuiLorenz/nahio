...
// --- NOVO COMPONENTE: Modal de Seleção (Genérico) ---
const SelectListModal = ({ visible, onClose, onSelect, data, title, keyExtractor, renderItemContent }) => {
      ...
      <View style={selectModalStyles.container}>
        <View style={selectModalStyles.content}>
          ...
          <FlatList
            ...
            ListEmptyComponent={<Text style={selectModalStyles.emptyText}>Nenhum item disponível.</Text>}
...
const ConvitesScreen = ({ navigation }) => {
  ...
  // Estados de dados para Novo Convite (Instituição)
  const [olheiros, setOlheiros] = useState([]);
  const [atletas, setAtletas] = useState([]);
  
  // Estados para seleção de modais
  ...
  // Use useCallback para otimização
  const loadConvites = useCallback(async () => {
    setLoading(true);
    try {
      let result;
      // Usar uma variável para o ID da instituição
      const instituicaoId = userData.userType === 'responsavel' 
      ...
      if (userData.userType === 'olheiro') {
        ...
        result = await ConviteService.buscarConvitesPorInstituicao(instituicaoId);
      } else {
        // Caso de erro para responsável sem instituicaoId
      ...
      if (result.success) {
        ...
        const sortedConvites = (result.convites || []).sort((a, b) => 
  ...
  const loadAtletas = useCallback(async () => {
    try {
      // Garantir que a instituiçãoId exista para a chamada
      const idToUse = userData.userType === 'responsavel' 
        ? userData.profile?.instituicaoId 
        : userData.uid;

      if (!idToUse) return; // Evita chamada desnecessária
  ...
  useEffect(() => {
    ...
    // Carrega dados de apoio apenas para quem pode enviar convites
    if (userData.userType === 'instituicao' || userData.userType === 'responsavel') {
      loadOlheiros();
      loadAtletas();
    }
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'aceito': return colors.success;
      case 'recusado': return colors.error;
  ...
  const handleAceitarConvite = async (conviteId) => {
    try {
      ...
      if (result.success) {
        Alert.alert('Sucesso', 'Convite aceito! O atleta foi adicionado à sua lista.');
        loadConvites();
  ...
  // Funções de seleção para os novos modais
  ...
  const handleCriarConvite = async () => {
    // Acesso ao ID da instituição via userData.uid ou userData.profile.instituicaoId
    const instituicaoId = userData.userType === 'responsavel' 
        ? userData.profile?.instituicaoId
        : userData.uid;
    ...
    // Garantir que apenas Instituição ou Responsável possam criar
    if (userData.userType !== 'instituicao' && userData.userType !== 'responsavel') {
    ...
    try {
      const conviteData = {
        ...
        olheiroId: selectedOlheiro.uid, // Supondo que o Olheiro tem 'uid'
        atletaId: selectedAtleta.id,   // Supondo que o Atleta tem 'id'
      ...
      if (result.success) {
        Alert.alert('Sucesso', 'Convite enviado com sucesso! Ele aparecerá como "Enviado" até ser respondido.');
        setNovoConviteModal(false);
        resetNovoConviteForm();
        loadConvites(); // Recarrega a lista
      } else {
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      {(userData.userType === 'instituicao' || userData.userType === 'responsavel') && (
        <TouchableOpacity 
  ...
  return (
    <SafeAreaView style={globalStyles.safeArea}>
      <View style={styles.container}>
              ...
              {selectedConvite && (
                <ScrollView style={styles.modalBody}>
                  ...
                  <View style={styles.detailItem}>
                    ...
                    <View style={[styles.statusBadge, { backgroundColor: getStatusColor(selectedConvite.status) }]}>
                  ...
                  <View style={styles.actionsContainer}>
                    {selectedConvite.status === 'enviado' && userData.userType === 'olheiro' && (
                      <>
                        <TouchableOpacity 
                          style={[styles.actionButton, styles.confirmButton]}
                          onPress={() => handleAceitarConvite(selectedConvite.id)}
                        >
                          <Text style={styles.actionButtonText}>Aceitar</Text>
                        </TouchableOpacity>
                        <TouchableOpacity 
                          style={[styles.actionButton, styles.cancelButton]}
                          onPress={() => handleRecusarConvite(selectedConvite.id)}
                        >
            ...
            <View style={styles.modalContent}>
              ...
              <ScrollView style={styles.modalBody}>
                ...
                <View style={styles.inputContainer}>
                  ...
                  >
                    <Text style={styles.selectButtonText}>
                      {selectedOlheiro ? selectedOlheiro.nome : 'Selecionar olheiro'}
                    </Text>
                  </TouchableOpacity>
                  {olheiros.length === 0 && (
                     <Text style={styles.selectErrorText}>Nenhum olheiro encontrado.</Text>
                  )}
                </View>
                ...
                <View style={styles.inputContainer}>
                  ...
                  >
                    <Text style={styles.selectButtonText}>
                      {selectedAtleta ? selectedAtleta.nome : 'Selecionar atleta'}
                    </Text>
                  </TouchableOpacity>
                  {atletas.length === 0 && (
                     <Text style={styles.selectErrorText}>Nenhum atleta cadastrado.</Text>
                  )}
                </View>
        ...
        {/* Modais de Seleção (Olheiro e Atleta) */}
        <SelectListModal
          ...
          renderItemContent={(item) => item.nome || `Olheiro #${item.uid.substring(0, 4)}`}
        />
        ...
        <SelectListModal
          ...
          keyExtractor={(item) => item.id}
          renderItemContent={(item) => `${item.nome} (${item.posicao || 'N/A'})`}
        />
      </View>
    </SafeAreaView>
  );
...
// --- Estilos para o Modal de Seleção (separado) ---
